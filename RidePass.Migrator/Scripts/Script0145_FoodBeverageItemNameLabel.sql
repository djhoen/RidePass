-- Display-text rename: the concession row in v_recent_sales surfaces an item name shown in the admin
-- Purchases list. Rebranded from "Concession (N items)" to "Food & Beverage (N items)". This recreates
-- the view from Script0142 changing ONLY that literal; the 'concession' kind key and all columns are
-- unchanged (the internal source kind stays 'concession').
