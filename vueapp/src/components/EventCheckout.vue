<template>
    <div class="evt-checkout">
        ...
        <template v-if="step === 'select'">
            ...
            <div v-if="showRiderCount" class="evt-riders mb-4 pa-3">
                <div class="d-flex align-center justify-space-between">
                    ...
                    <div class="d-flex align-center ga-1">
                        <v-btn icon="mdi-minus" size="x-small" variant="tonal" :disabled="riderCount <= 1" @click="riderCount = Math.max(1, riderCount - 1)"></v-btn>
            ...
            <template v-if="hasSelection">
                ...
                <p class="text-caption text-medium-emphasis mb-3">Final total, including any code, is shown before you pay.</p>
            </template>
        ...
        <template v-else-if="step === 'details'">
            ...
            <v-text-field v-model="name" label="Full name" density="compact" class="mb-3 mt-4" :readonly="isAuthed"></v-text-field>
            ...
            <template v-if="orderLines.length">
                ...
                <p class="text-caption text-medium-emphasis mt-1">Final total, including any code, is shown before you pay.</p>
            </template>
...
// Show the "number of riders" stepper only for a race with more than one class entry.
const showRiderCount = computed(() => isRaceEvent.value && totalRaceQty.value > 1)
// Riders implied by the cart: the stepper for a multi-class race, else 1 per class entry.
const raceRiderCount = computed(() => {
    if (!isRaceEvent.value) return 0
    if (totalRaceQty.value <= 1) return totalRaceQty.value
    return riderCount.value
})
...
// Keep riderCount sane as the cart changes.
watch(totalRaceQty, (n) => {
    if (riderCount.value > n) riderCount.value = Math.max(1, n)
    if (riderCount.value < 1) riderCount.value = 1
})
